...
allattr_list = [] #list of All attributtes and their output values
for j in range(len(attributes)-1):
	attri = []
	for i in range(len(data)):
		vals = []
		vals.append(data[i][j])
		vals.append(data[i][len(attributes)-1])
		attri.append(vals)
	attri.sort()
...
#for k in range(len(allattr_list)): 
maxindex = 0
...
attr.sp = spl
splitlist(attr,attribute_list,spl)
tree.append(attr)

attribute_list.remove(attribute_list[maxindex])

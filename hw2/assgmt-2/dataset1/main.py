...
def get_objective_function(trainx,trainy,loss_type, regularizer_type, loss_weight):
    ...
    loss_function = utils1.loss_functions[loss_type]
    if regularizer_type != None:

        regularizer_function = utils1.regularizer_functions[regularizer_type]
    def objective_function(weights):
        ...
        if regularizer_type != None:
            # regulariser function is called from utils1.py
...
def get_gradient_function(trainx,trainy,loss_type, regularizer_type, loss_weight):
    ...
    loss_grad_function = utils1.loss_grad_functions[loss_type]
    if regularizer_type != None:
        regularizer_grad_function = utils1.regularizer_grad_functions[regularizer_type]
    ...
    def gradient_function(weights):
        ...
        # loss_grad_function is called from utils1.py    
        ...
        if regularizer_type != None:
            # regulariser grad function is called from utils1.py
            gradient += regularizer_grad_function(weights)
        return gradient
    return gradient_function

def train(data_loader, loss_type, regularizer_type, loss_weight, num_ep):
...
def main(args):
	...
	for c in range(4):
		...
		for ne in [1, 100]:# , 500, 750, 1500]:
			...
			trained_model_parameters = train(train_data_loader, args.loss_type, args.regularizer_type, args.loss_weight, args.num_ep)
	...
	epoch = [1, 100]# , 500, 750, 1500]

	for i in range(len(noea)):
		plt.plot(epoch,noea[i],'b-o')
		plt.title( 'loss : {0} , regulariger : {1}'.format(args.loss,args.regularizer))
...
if __name__=="__main__":
    ...
    parser.add_argument("--num_ep", action="store", dest="num_ep", type=int, help="number of traning itterations", default=1000)      

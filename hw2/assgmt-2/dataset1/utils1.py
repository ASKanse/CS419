...
def square_hinge_loss(targets, outputs):
	...
	sum1 = 0.0
	for i in range(l):
		if((targets[i]-0.5)*(outputs[i]) <= 1):
			sum1+= (1-(targets[i]-0.5)*(outputs[i]))**2
	return sum1/l
...
def logistic_loss(targets, outputs):
	...
	sum1 = 0.0
	for i in range(l):
		sum1+= math.log(1+math.exp(-(targets[i]-0.5)*(outputs[i])))
	return sum1/l
...
def square_hinge_grad(weights,inputs, targets, outputs):
  # Write thee square hinge loss gradient here
	...
	for j in range(l1):
		...
		for i in range(l):
			if((targets[i]-0.5)*(outputs[i]) <= 1):
				sum1+= -2*(targets[i]-0.5)*inputs[i][j]*(1-(targets[i]-0.5)*(outputs[i]))
		result.append(sum1)
	return result
...
def logistic_grad(weights,inputs, targets, outputs):
	...
	for j in range(l1):
		sum1 = 0
		for i in range(l):
			sum1 += (math.exp(- (targets[i]-0.5) * (outputs[i]))/(1 + math.exp(- (targets[i]-0.5) * (outputs[i]))))*(-inputs[i][j]*(targets[i]-0.5))
		results.append(sum1)
	return results
